...
from .harassment_analyzer import CriterionResult, HarassmentAnalysisResponse
...
class HarassmentAnalyzer:
    ...
    def _calculate_total_score(self, criteria: list[CriterionResult]) -> int:  # type: ignore
        weights = {
            "우위성": 0.22,
            "업무 관련성": 0.18,
            "업무상 적정범위 초과": 0.25,
    ...
    def _collect_evidence(self, criteria: list[CriterionResult]) -> list[str]: # type: ignore
        evidence = []
        for item in criteria:
            if item.evidence:
